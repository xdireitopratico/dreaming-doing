/** Barrel do chat Lovable — importe daqui, não de arquivos soltos. */
export { ChatPanel } from "./ChatPanel";
export type { ChatPanelProps } from "./ChatPanel";
export { ChatThread } from "./ChatThread";
export { ChatMessage } from "./ChatMessage";
export { ChatComposer } from "./ChatComposer";
export { ComposerMorePopover } from "./ComposerMorePopover";
export { HotlOperationControl } from "./HotlOperationControl";
export { ForgePopoverShell } from "./ForgePopoverShell";
export { AssistantTurn } from "./AssistantTurn";
export { ChatClarifyDock } from "./ChatClarifyDock";
export type { ChatClarifyDockProps } from "./ChatClarifyDock";
export { ChatUserBubble } from "./ChatUserBubble";
export { ChatNarration } from "./ChatNarration";

export { ChatJobCard } from "./ChatJobCard";
export { ChatJobTasksCard } from "./ChatJobTasksCard";
export { ChatToolbar } from "./ChatToolbar";
