/**
 * NodeIcon — n8n-style node icon renderer
 *
 * Supports three source types:
 *   "file"  → <img> with optional filter
 *   "icon"  → inline SVG from NAMED_ICONS registry
 *   "unknown" → fallback letter tile
 *
 * Context-based sizing (n8n-compatible):
 *   canvas        → 48px
 *   configuration → 36px
 *   nodeList      → 24px
 *   ndvHeader     → 24px
 *
 * Each source can carry an optional "badge" (sub-icon rendered top-right).
 */
import { type FC } from "react";

// ── Types ──

type BaseNodeIconSource =
  | { type: "file"; src: string }
  | { type: "icon"; name: string; color: string }
  | { type: "unknown"; label: string };

export type NodeIconSource = BaseNodeIconSource & {
  badge?: BaseNodeIconSource;
};

export type NodeIconContext = "canvas" | "configuration" | "nodeList" | "ndvHeader";

const ICON_SIZES: Record<NodeIconContext, number> = {
  canvas: 48,
  configuration: 36,
  nodeList: 24,
  ndvHeader: 24,
};

export function getNodeIconSize(context: NodeIconContext): number {
  return ICON_SIZES[context];
}

// ── Theme-aware color ──
const NODE_COLORS: Record<string, string> = {
  trigger: "#34d399",
  llm: "var(--ps-accent, #3b82f6)",
  tool: "#eab308",
  condition: "#6b7280",
  output_guard: "#f59e0b",
  stt: "#a855f7",
  tts: "#f97316",
  rag_search: "#b45309",
  memory: "#ec4899",
  hitl: "#ef4444",
  loop: "#64748b",
  sub_flow: "#1f2937",
  delay: "#9ca3af",
  error_handler: "#dc2626",
  switch: "#6366f1",
  transformer: "#06b6d4",
  vision: "#7c3aed",
};

/** Node color for a given type — falls back to CSS var or hex. */
export function getNodeIconColor(nodeType: string): string {
  return NODE_COLORS[nodeType] ?? "#6b7280";
}

// ── Source resolver ──

export function getNodeIconSource(nodeType: string): NodeIconSource {
  const map: Record<string, NodeIconSource> = {
    trigger: { type: "icon", name: "zap", color: "#34d399" },
    llm: { type: "icon", name: "bot", color: "var(--ps-accent, #3b82f6)" },
    tool: { type: "icon", name: "tool", color: "#eab308" },
    condition: { type: "icon", name: "git-branch", color: "#6b7280" },
    output_guard: { type: "icon", name: "shield", color: "#f59e0b" },
    stt: { type: "icon", name: "mic", color: "#a855f7" },
    tts: { type: "icon", name: "volume-2", color: "#f97316" },
    rag_search: { type: "icon", name: "search", color: "#b45309" },
    memory: { type: "icon", name: "brain", color: "#ec4899" },
    hitl: { type: "icon", name: "users", color: "#ef4444" },
    loop: { type: "icon", name: "repeat", color: "#64748b" },
    sub_flow: { type: "icon", name: "package", color: "#1f2937" },
    delay: { type: "icon", name: "clock", color: "#9ca3af" },
    error_handler: { type: "icon", name: "alert-triangle", color: "#dc2626" },
    switch: { type: "icon", name: "git-branch", color: "#6366f1" },
    transformer: { type: "icon", name: "settings", color: "#06b6d4" },
    vision: { type: "icon", name: "eye", color: "#7c3aed" },
  };
  return map[nodeType] || { type: "unknown", label: nodeType };
}

// ── Component ──

interface NodeIconProps {
  source: NodeIconSource;
  size?: number;
  /** If true, img icons get brightness(0) invert(1) filter (dark-bg optimised). */
  invertDark?: boolean;
}

export function NodeIcon({ source, size = 20, invertDark = true }: NodeIconProps) {
  return (
    <div className="relative inline-flex shrink-0">
      {source.type === "file" ? (
        <img
          src={source.src}
          alt=""
          width={size}
          height={size}
          className="shrink-0"
          style={invertDark ? { filter: "brightness(0) invert(1)" } : undefined}
        />
      ) : source.type === "icon" ? (
        <NamedIcon name={source.name} color={source.color} size={size} />
      ) : (
        <div
          className="shrink-0 rounded flex items-center justify-center text-[9px] font-bold uppercase"
          style={{
            width: size,
            height: size,
            background: "rgba(255,255,255,0.1)",
          }}
        >
          {source.label.slice(0, 2)}
        </div>
      )}

      {/* Badge (sub-icon) — top-right 8px */}
      {source.badge && (
        <div className="absolute -top-1 -right-1 z-10">
          {source.badge.type === "file" ? (
            <img src={source.badge.src} alt="" width={10} height={10} className="rounded-sm" />
          ) : "color" in source.badge ? (
            <div
              className="rounded-sm flex items-center justify-center text-[6px] font-bold"
              style={{
                width: 10,
                height: 10,
                background: `${source.badge.color}33`,
                color: source.badge.color,
              }}
            >
              {"name" in source.badge ? source.badge.name.slice(0, 1) : ""}
            </div>
          ) : null}
        </div>
      )}
    </div>
  );
}

// ── Inline SVG named icons (n8n-style minimal) ──

const NAMED_ICONS: Record<string, FC<{ size: number }>> = {
  zap: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" />
    </svg>
  ),
  bot: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect x="3" y="11" width="18" height="10" rx="2" /><circle cx="12" cy="5" r="2" /><path d="M12 7v4" /><line x1="8" y1="16" x2="8" y2="16" /><line x1="16" y1="16" x2="16" y2="16" />
    </svg>
  ),
  tool: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z" />
    </svg>
  ),
  "git-branch": ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <line x1="6" y1="3" x2="6" y2="15" /><circle cx="18" cy="6" r="3" /><circle cx="6" cy="18" r="3" /><path d="M18 9a9 9 0 0 1-9 9" />
    </svg>
  ),
  shield: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>
  ),
  mic: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" />
    </svg>
  ),
  "volume-2": ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5" /><path d="M19.07 4.93a10 10 0 0 1 0 14.14" /><path d="M15.54 8.46a5 5 0 0 1 0 7.07" />
    </svg>
  ),
  search: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" />
    </svg>
  ),
  brain: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M9.5 2A2.5 2.5 0 0 1 12 4.5v15a2.5 2.5 0 0 1-4.96.44 2.5 2.5 0 0 1-2.96-3.08 3 3 0 0 1-.34-5.58 2.5 2.5 0 0 1 1.32-4.24 2.5 2.5 0 0 1 4.44-2.04Z" /><path d="M14.5 2A2.5 2.5 0 0 0 12 4.5v15a2.5 2.5 0 0 0 4.96.44 2.5 2.5 0 0 0 2.96-3.08 3 3 0 0 0 .34-5.58 2.5 2.5 0 0 0-1.32-4.24 2.5 2.5 0 0 0-4.44-2.04Z" />
    </svg>
  ),
  users: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M22 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" />
    </svg>
  ),
  repeat: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <polyline points="17 1 21 5 17 9" /><path d="M3 11V9a4 4 0 0 1 4-4h14" /><polyline points="7 23 3 19 7 15" /><path d="M21 13v2a4 4 0 0 1-4 4H3" />
    </svg>
  ),
  package: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <line x1="16.5" y1="9.4" x2="7.5" y2="4.21" /><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" /><polyline points="3.27 6.96 12 12.01 20.73 6.96" /><line x1="12" y1="22.08" x2="12" y2="12" />
    </svg>
  ),
  clock: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" />
    </svg>
  ),
  "alert-triangle": ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" />
    </svg>
  ),
  settings: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z" />
    </svg>
  ),
  eye: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" />
    </svg>
  ),
};

function NamedIcon({ name, color, size }: { name: string; color: string; size: number }) {
  const IconComponent = NAMED_ICONS[name];
  if (!IconComponent) {
    return (
      <div
        className="shrink-0 rounded flex items-center justify-center text-[9px] font-bold uppercase"
        style={{ width: size, height: size, background: `${color}33`, color }}
      >
        {name.slice(0, 2)}
      </div>
    );
  }
  return (
    <div className="shrink-0 rounded flex items-center justify-center" style={{ color }}>
      <IconComponent size={size} />
    </div>
  );
}
