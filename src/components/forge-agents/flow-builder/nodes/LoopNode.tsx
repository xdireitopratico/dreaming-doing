...
export function LoopNode({ data, selected }: NodeProps) {
  ...
  return <BaseNode selected={selected} status={resolveNodeStatus(data)} icon={getNodeIconSource("loop")} label="Loop" subtitle={`max: ${config.max_iterations ?? 10} iterações`} />;
}
