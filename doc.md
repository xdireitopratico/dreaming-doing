...
| 2 | **+1** | ✅ | Modal usa `createProjectFromPrompt` (server fn); Edge `create-project` removida do app | `template` + `firstPrompt` no input | `CreateProjectDialog.tsx`, `projects.functions.ts` |
| 3 | **+2**, **E36** | ✅ | `publishProject` invoca Edge `deploy-publish` (`deployTarget` na Edge) | — | `publish.functions.ts`, `deploy-publish/index.ts` |
| 4 | **+4** | ✅ | `/deploy` no chat chama `handlePublish` | — | `ChatInput.tsx`, `projects/$projectId/index.tsx` |
| 5 | **B13** | 👤 | Checklist em `secrets-checklist.md`; rotação manual pós-migrate | Reset key + `secrets set` + `deploy-all.sh` | Dashboard Supabase ambos refs |
...
**Próximo:** P0 #1 sync (A6/A2) + P1 agente (C14+).
