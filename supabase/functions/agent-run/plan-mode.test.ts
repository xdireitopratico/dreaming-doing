...
import { buildProposedPlan, extractRationaleFromLlmContent } from "./plan-mode.ts";
  ...
  const plan = buildProposedPlan(
    {
      ...
      plan: {
        ...
        steps: [
          { id: "s1", type: "create_file", description: "Criar arquivo A", filePath: "a.ts", enabled: true },
          { id: "s2", type: "edit_file", description: "Editar arquivo B", filePath: "b.ts", enabled: true },
        ],
      },
    },
    null,
...
Deno.test("buildProposedPlan: caminho 2 — extrai do rawContent quando classification.plan ausente", () => {
  const raw = JSON.stringify({
    ...
    plan: {
      rationale: "Do raw",
      steps: [
        { id: "s1", type: "edit_file", description: "Editar Y", filePath: "y.ts" },
      ],
    },
  });
  const plan = buildProposedPlan(
    { type: "modify", summary: "Ajuste X" },
    raw,
    opts,
  );
...
Deno.test("buildProposedPlan: caminho 3 — sem LLM estruturado, usa deriveDefaultPlan + rationale genérico", () => {
  const plan = buildProposedPlan(
  ...
  const plan = buildProposedPlan(
    {
      type: "modify",
      summary: "x",
      plan: { rationale: "", steps: [{ id: "s1", type: "custom", description: "d", enabled: true }] },
    },
    null,
    opts,
  );
...
Deno.test("extractRationaleFromLlmContent: parseia {plan:{rationale,steps[]}}", () => {
  const raw = JSON.stringify({ plan: { rationale: "r", steps: [{ type: "custom", description: "d" }] } });
