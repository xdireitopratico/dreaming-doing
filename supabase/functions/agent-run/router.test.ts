...
import {
  assertEquals, assertExists, assert,
...
Deno.test("router.classify: LLM devolve plan estruturado → classification.plan populado", async () => {
  const llm = new MockLLM([
    mockResp(JSON.stringify({
      complexity: 3,
      type: "new_project",
      ...
      needsDeps: true,
      plan: {
        rationale: "Vamos começar pela estrutura do componente e os tipos pra você revisar antes da lógica de animação. Depois faço os estilos e por fim a animação — assim se algo precisar mudar, ajustamos cedo.",
        steps: [
          { id: "s1", type: "create_file", description: "Criar src/components/Toast.tsx com props variant/message/duration e types", filePath: "src/components/Toast.tsx", estimatedCost: 0.003 },
          { id: "s2", type: "create_file", description: "Criar src/components/Toast.module.css com as 4 variantes de cor (success/error/warning/info)", filePath: "src/components/Toast.module.css", estimatedCost: 0.002 },
          { id: "s3", type: "install_dep", description: "Instalar framer-motion para a animação de entrada/saída", estimatedCost: 0.001 },
          { id: "s4", type: "shell_exec", description: "Rodar npm run typecheck pra validar que não quebrou o build", estimatedCost: 0.001 },
        ],
      },
    })),
  ]);
  ...
  assertEquals(result.summary, "Vou criar um componente de Toast com 4 variantes visuais e animação de entrada");
...
Deno.test("router.classify: LLM sem plan → classification.plan é undefined, summary mantido", async () => {
  const llm = new MockLLM([
    mockResp(JSON.stringify({
      complexity: 2,
      type: "modify",
...
Deno.test("router.classify: step inválido (sem description) é pulado, mas steps válidos sobrevivem", async () => {
  const llm = new MockLLM([
    mockResp(JSON.stringify({
      complexity: 2,
      type: "modify",
...
Deno.test("router.classify: usa cheap model (max_tokens >= 1000)", async () => {
  const llm = new MockLLM([mockResp(JSON.stringify({ complexity: 1, type: "other", summary: "x", needsBuild: false, needsDeps: false }))]);
  const r = routerWith(llm);
  await r.classify("oi", "");
  assertEquals(llm.calls.length, 1);
  const call = llm.calls[0];
  assertExists(call.max_tokens);
  assert(call.max_tokens >= 1000, `max_tokens=${call.max_tokens} deveria ser >= 1000 pra caber o plano`);
});
...
Deno.test("router.classify: temperature baixa (0.0-0.3) para planos determinísticos", async () => {
  ...
  assert(t !== undefined && t >= 0 && t <= 0.3, `temperature=${t} deveria ser baixa pra planos estáveis`);
});
