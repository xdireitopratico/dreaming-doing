    ...
    if (action === "export") {
      ...
      if (body.confirm !== "DELETE_ALL_MY_DATA") {
        return new Response(JSON.stringify({ 
          error: "Deletion requires confirmation. Send { action: 'delete', confirm: 'DELETE_ALL_MY_DATA' }",
      ...
      return new Response(JSON.stringify({ error: "Invalid action. Use: export, delete, summary" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
  } catch (err: any) {
...
async function handleSummary(supabase: any, userId: string): Promise<Response> {
  ...
  for (const table of ["agent_flows", "agent_executions", "agent_deployments", "agent_marketplace_listings"]) {
    // BUG 76 FIX: Correct column per table
    const col = table === "agent_marketplace_listings" ? "publisher_id" : "user_id";
    const { count } = await supabase.from(table).select("id", { count: "exact", head: true }).eq(col, userId);
...
async function handleExport(supabase: any, userId: string): Promise<Response> {
  ...
  const { data: flows } = await supabase.from("agent_flows").select("*").eq("user_id", userId);
  exportData.agent_flows = flows || [];
  const flowIds = (flows || []).map((f: any) => f.id);

  if (flowIds.length > 0) {
    for (const table of ["agent_flow_nodes", "agent_flow_versions", "agent_deployments", "agent_executions", "agent_test_suites", "agent_schedules", "agent_marketplace_listings"]) {
      ...
      const pageSize = 1000;
      while (true) {
        const { data } = await supabase.from(table).select("*").in(col, flowIds).range(offset, offset + pageSize - 1);
    ...
    if (execIds.length > 0) {
      ...
      let offset = 0;
      while (true) {
        const { data: steps } = await supabase.from("agent_execution_steps").select("*").in("execution_id", execIds).range(offset, offset + 999);
    ...
    const { data: ragDocs } = await supabase.from("rag_documents").select("*").in("flow_id", flowIds);
    exportData.rag_documents = ragDocs || [];
    
    const { data: secrets } = await supabase.from("tenant_secrets").select("id, secret_name, created_at").eq("user_id", userId);
    exportData.tenant_secrets = (secrets || []).map((s: any) => ({ ...s, secret_value: "[REDACTED]" }));
  }

  const { data: notifications } = await supabase.from("agent_notifications").select("*").eq("user_id", userId);
...
async function handleDelete(supabase: any, userId: string, email: string): Promise<Response> {
  ...
  const { data: flows } = await supabase.from("agent_flows").select("id").eq("user_id", userId);
  const flowIds = (flows || []).map((f: any) => f.id);

  if (flowIds.length > 0) {
    // BUG 36 FIX: Get execution IDs scoped to user's flows only
    const { data: userExecs } = await supabase.from("agent_executions").select("id").in("flow_id", flowIds);
    ...
    if (userExecIds.length > 0) {
      const { error } = await supabase.from("agent_execution_steps").delete().in("execution_id", userExecIds);
    ...
    const { data: ragDocs } = await supabase.from("rag_documents").select("id").in("flow_id", flowIds);
    const ragDocIds = (ragDocs || []).map((d: any) => d.id);
    if (ragDocIds.length > 0) {
      await supabase.from("rag_chunks").delete().in("document_id", ragDocIds);
      deletedTables.push("rag_chunks");
    }
    if (ragDocs?.length) {
      await supabase.from("rag_documents").delete().in("flow_id", flowIds);
      deletedTables.push("rag_documents");
    }

    // Delete other flow-scoped tables
    const flowTables = [
      "agent_executions", "agent_flow_nodes", "agent_flow_versions",
      "agent_flow_comments", "agent_flow_members", "agent_deployments",
      "agent_test_suites", "agent_schedules", "agent_alert_rules",
    ];
    ...
    // Marketplace
    const { data: listings } = await supabase.from("agent_marketplace_listings").select("id").in("flow_id", flowIds);
    const listingIds = (listings || []).map((l: any) => l.id);
    if (listingIds.length > 0) {
      await supabase.from("agent_marketplace_ratings").delete().in("listing_id", listingIds);
      await supabase.from("agent_marketplace_purchases").delete().in("listing_id", listingIds);
      deletedTables.push("agent_marketplace_ratings", "agent_marketplace_purchases");
    }
    await supabase.from("agent_marketplace_listings").delete().in("flow_id", flowIds);
    deletedTables.push("agent_marketplace_listings");
  }
  ...
  // User-scoped tables
  await supabase.from("agent_notifications").delete().eq("user_id", userId);
  deletedTables.push("agent_notifications");

  // BUG 37 FIX: Delete tenant secrets
  await supabase.from("tenant_secrets").delete().eq("user_id", userId);
  deletedTables.push("tenant_secrets");
  ...
  if (flowIds.length > 0) {
    await supabase.from("webhook_inbox").delete().in("external_id", flowIds);
    deletedTables.push("webhook_inbox");
  }

  console.log(`[GDPR] Data deletion completed for user ${userId}. Tables: ${deletedTables.join(", ")}`);
