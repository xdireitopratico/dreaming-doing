...
async function treatPromptRewrite(
  ...
  const llmNodes = nodes.filter(
    (n: any) => n.type === "llm" || n.type === "ai-agent" || n.data?.systemPrompt,
  );
  ...
  const targetNode =
    (symptomNodeId && llmNodes.find((n: any) => n.id === symptomNodeId)) || llmNodes[0];
  const originalPrompt =
    targetNode.data?.systemPrompt || targetNode.data?.config?.systemPrompt || "";
...
async function treatTimeoutAdjust(
  ...
  if (shadowMode) {
    return {
      success: true,
      data: { shadow: true, current_timeout_ms: currentTimeout, new_timeout_ms: newTimeout },
    };
  }
  ...
  return {
    success: !error,
    data: { current_timeout_ms: currentTimeout, new_timeout_ms: newTimeout, applied: true },
  };
}
...
async function treatCacheClear(
  ...
  const { error, count } = await sb.from("semantic_cache").delete().eq("flow_id", flowId);
...
async function treatRollback(
  ...
  if (shadowMode) {
    return {
      ...
      data: {
        shadow: true,
        rollback_to_version: previousVersion.version_number,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!modelId) {
      return new Response(
        JSON.stringify({ error: "model_id is required (zero-llm-hardcoded policy)" }),
        {
          status: 400,
          headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
        },
    ...
    console.log(
      "[physician] Starting diagnostic sweep...",
      targetFlowId ? `flow=${targetFlowId}` : "all enabled flows",
    );

    let configQuery = sb.from("prometheus_auto_heal_config").select("*").eq("enabled", true);
    ...
    if (!configs?.length) {
      return new Response(
        JSON.stringify({ ok: true, diagnosed: 0, treated: 0, reason: "no_configs" }),
        {
          headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
        },
      );
    ...
    for (const config of configs as HealConfig[]) {
      ...
      if (errorRate > config.error_spike_threshold) {
        symptoms.push({
          ...
          data: {
            error_rate: errorRate,
            threshold: config.error_spike_threshold,
            error_count: errors.length,
            sample_errors: errors.slice(0, 3).map((e) => e.error_message || "unknown"),
          },
        });
      }
      ...
      if (avgQuality != null && avgQuality < config.quality_drop_threshold) {
        symptoms.push({
          ...
          data: {
            avg_quality: avgQuality,
            threshold: config.quality_drop_threshold,
            sample_count: qualityScores.length,
          },
        });
      }
      ...
      if ((recentCorrections || 0) >= config.max_auto_corrections) {
        console.log(
          `[physician] Flow ${config.flow_id}: max corrections (${config.max_auto_corrections}) reached in 24h. Skipping treatment.`,
        );
        // Still log diagnosis
        for (const s of symptoms) {
          await sb.from("prometheus_healing_log").insert({
            ...
            symptom: s.symptom,
            symptom_data: s.data,
            diagnosis: "Max corrections reached",
            severity: "high",
            outcome: "max_corrections_reached",
      ...
      for (const symptom of symptoms) {
        ...
        try {
          ...
          await sb.from("prometheus_healing_log").insert({
            ...
            model_used: modelId,
            diagnosis_latency_ms: diagLatency,
            user_id: config.user_id,
            resolved_at:
              treatmentResult?.success && !config.shadow_mode ? new Date().toISOString() : null,
          });
        } catch (err: any) {
          ...
          await sb.from("prometheus_healing_log").insert({
            ...
            metrics_before: metricsBefore,
            model_used: modelId,
            diagnosis_latency_ms: Date.now() - diagStart,
            user_id: config.user_id,
          });
        }
      }
...
async function applyTreatment(
  ...
  console.log(
    `[physician] Applying treatment '${treatment}' to flow ${flowId} (shadow=${shadowMode})`,
  );
